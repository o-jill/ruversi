            ...
            loop {
                match frmain2.recv() {
                    Ok( rfenidxgrp ) => {
                        ...
                        // wait updating
                        {let _uu = cdvar.wait_while(status.lock().unwrap(), |upd| {
                            *upd != 0
                        }).unwrap();}
                        ...
                        {
                            let mut upd = status.lock().unwrap();
                            // weight.updatemb(&bufweight, n * 2);
                            *upd = 1;
                            cdvar.notify_one();
                        }
                    },
